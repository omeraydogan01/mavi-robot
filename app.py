...
from openai import OpenAI  # Yeni API
...
def main():
    ...
    if user_question:
        docs_text = []
        if uploaded_files:
            docs_text = vectorstore.similarity_search(user_question, k=6)

        images_info = []
        if uploaded_images:
            for img in uploaded_images:
                try:
                    # Görseli OpenAI ile analiz etme
                    response = client.chat.completions.create(
                        model="gpt-4.1-mini",
                        messages=[
                            {"role": "user", 
                             "content": f"Bu görselle ilgili soruyu yanıtla: {user_question}"}
                        ],
                        files=[img]
                    )
                    img_answer = response.choices[0].message.content
                    images_info.append(img_answer)
                except Exception as e:
                    images_info.append(f"🛑 Görsel analizi sırasında hata oluştu: {e}")

        # Text ve görsel cevapları birleştirme
        combined_answer = ""
        if docs_text:
            llm_chain = ChatOpenAI(model="gpt-4o-mini", temperature=0, api_key=api_key)
            chain = load_qa_chain(llm_chain, chain_type="stuff")
            text_answer = chain.run(input_documents=docs_text, question=user_question)
            combined_answer += f"{text_answer}\n"
        if images_info:
            combined_answer += "\n".join(images_info)
